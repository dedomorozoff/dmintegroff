...
- **История запросов**: Отправленные запросы с полными ответами и временем выполнения
...
- **Кастомные запросы**: Полная настройка HTTP запросов с произвольными заголовками, методами и телом
...
- **Отправленные запросы**: История ваших тестовых и кастомных запросов с ответами
- **Полученные запросы**: Все входящие запросы в реальном времени
...
Нажмите кнопку "Тестовый запрос" на странице вебхука. Будет отправлен POST запрос с тестовыми данными:
...
#### Кастомный запрос
Нажмите кнопку "Кастомный запрос" для полной настройки HTTP запроса **на любой URL**:

- **URL назначения**: Любой URL (тестовый вебхук, внешние API, тестовые сервисы)
- **HTTP методы**: GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS
- **Заголовки**: Добавление произвольных заголовков (по умолчанию: Content-Type, User-Agent: DmIntegroff)
- **Query параметры**: URL параметры
- **Тело запроса**: Поддержка JSON, XML, Form Data, Text с шаблонами
- **Быстрые кнопки**: HTTPBin, JSONPlaceholder, Postman Echo для тестирования
- **Без CORS**: запросы через серверный прокси с полными ответами

Подробнее: [Кастомные запросы](./WEBHOOK_TEST_CUSTOM_REQUESTS.md)
