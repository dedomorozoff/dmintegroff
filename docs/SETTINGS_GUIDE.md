...
### APP_URL
Базовый URL приложения, используется для генерации webhook URL.
...
**Значения:**
- `http://localhost:8080` - для локальной разработки (по умолчанию)
- `https://yourdomain.com` - для production
- `http://192.168.1.100:8080` - для доступа по локальной сети
