...
**Что делает:**
- Создает все таблицы (users, projects, integrations, request_logs)
- Автоматически добавляет новые колонки при обновлении структуры
- Создает индексы для оптимизации запросов
- Создает администратора по умолчанию (admin/admin)

**Когда использовать:**
- При первом развертывании
- После обновления структуры БД (например, добавление новых полей)
- При переходе на новую БД
- После обновления приложения с изменениями в моделях
- После `git pull` с изменениями в моделях данных

**Важно:** После миграции нужно пересобрать и перезапустить приложение:
```bash
...
💡 Примечание: CLI использует GORM AutoMigrate, который:
