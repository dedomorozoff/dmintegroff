...
// GetQuickSuggestions возвращает быстрые предложения для UI
func GetQuickSuggestions() []string {
	return []string{
		"Отправлять уведомления в Slack о новых заказах",
		"Создавать лиды в CRM при заполнении формы",
		"Отправлять сообщения в Telegram при ошибках",
		"Уведомлять в Discord о новых пользователях",
		"Отправлять email через SendGrid",
		"Синхронизировать данные с Google Sheets",
		"Создавать задачи в Jira при багах",
		"Отправлять метрики в аналитику",
	}
}

// GetPopularAPIs возвращает список популярных API
	...
	return []PopularAPI{
		{
			...
			AuthType:    "webhook",
			Description: "Отправка сообщений в Slack каналы",
